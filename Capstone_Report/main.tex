...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Diagrams and Images/driverPCB.png}
    \caption[PCB]{High level driving circuit PCB}
    \label{fig:kiCadPCB}
\end{figure}

\textbf{Power}\\
Each current driver requires two voltage supply levels to support its operation. The electromagnets must be supplied with high enough voltage to deliver currents in the range of 10-15A (based on magnet testing and determined magnet force requirements) through a \~1 $\Omega$ load (electromagnet series resistance). Additionally, according to Equation \ref{eqn:voltage_eq} a higher supply voltage will increase the rate at which current can rise through the electromagnets, improving responsiveness. To balance battery size and performance, a 14.7 V rail was determined to be sufficient. A 3.3 V supply is also required to power the current sense amplifier. This is supplied by the ESP32's internal voltage regulator, to ensure proper referenceing between the amplifier and the microcontroller.

\textbf{Switching Stage}\\
To switch the MOSFET quickly and efficiently, we placed a gate driver between the controller and the gate of each MOSFET. The gate driver provides sufficient current to charge and discharge the gate quickly so that the MOSFET spends less time in the region between being fully on and fully off, minimizing heat dissipation and losses. It also protects the microcontroller from having to source or sink more current than it is rated for. For this we used the TI UCC2742 as it provided a dual channel driver with performance that met our requirements. The switching frequency was chosen to balance losses and current ripple. A low frequency would allow current to rise and fall significantly between PWM cycles, reducing control precision. We found 4~kHz gave good control resolution without significant heating. The MOSFET showed notable heating above 10~kHz.
