...
Our selected IMU, provides us with current orientation "game vector" ($q$) every $100 \text{Hz}$. The game vector is sent over $I^2C$. With the quaternion representing the current orientation of the ball, we can determine the location of the magnets relative to the origin of the ball. Critically using the quaternion allows us to map the coordinates of the magnets in the ball’s reference frame to the world reference frame, which is critical allows to align the magnet locations with the joystick input. The rotation of a vector $\vec{v}$ by a quaternion $q$ can be calculated using the following formula:
...
Given a magnet and a current setpoint, we use a simple PI controller to select the appropriate duty cycle for the current. Since we are often required to control the current through multiple magnets simultanesly, we run a series of PI controllers in series. These series operation allows us to not overload the SPI and I2C communication lines. The series approach doubled our control loop time but we found that we were able to achieve stable control since the system dynamics are governed by intertia and not speed of the control loop.

\subsubsection{Active State}
\label{sec:active}
In the active state, our ESP runs 2 seperate processes, one on each core on the esp. One core is responable for receiving and sending data to the dashboard. The other core is responsible for the orientation tracking, magnet selection, and magnet current control. 
