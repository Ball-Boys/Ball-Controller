...
To actuate the required electromagnets, $\vec{F}_{\text{world}}$ and the global gravity vector are first transformed into the ball's local body frame ($\vec{F}_{\text{body}}$ and $\vec{g}_{\text{body}}$) using the rotation matrix derived from the IMU's quaternion.

For each magnet, the algorithm evaluates its angle relative to $\vec{g}_{\text{body}}$ to determine available torque capacity, and projects its force vector orthogonally to gravity ($\vec{v}_i$). To find the optimal combination of magnets to match the desired force, the system evaluates both individual candidates and pairs by minimizing a cost function. This function balances the target force error against an electrical current ($I_j$) penalty ($\lambda$):
\begin{equation}
    \text{Score} = \left\| \vec{F}_{\text{body}} - \sum_{j} (\vec{v}_j I_j) \right\| + \lambda \sum_{j} I_j
\end{equation}

The combination (single or pair) that yields the lowest score is selected for activation. Finally, to protect the hardware and ensure predictable linear behavior, all commanded currents are strictly capped at the core's maximum saturation limit ($I_{\text{max}}$).
